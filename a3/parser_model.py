...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        w = torch.zeros(size=(self.embed_size*self.n_features,self.hidden_size))
        
        w = nn.init.xavier_uniform_(torch.zeros((self.embed_size*self.n_features,self.hidden_size)))
        self.embed_to_hidden_weight = nn.Parameter(w)
        b = nn.init.uniform_(torch.zeros(self.hidden_size))
        self.embed_to_hidden_bias = nn.Parameter(b)
        self.dropout = nn.Dropout(self.dropout_prob)
        w = nn.init.xavier_uniform_(torch.zeros((self.n_classes,self.hidden_size)))
        self.hidden_to_logits_weight = nn.Parameter(w)
        b = nn.init.uniform_(torch.zeros(self.n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings[w].view(size=(w.shape[0], self.n_features*self.embed_size))
    ...
    def forward(self, w):
        ...
        f = nn.ReLU()
        x = f(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        x = self.dropout(x)
