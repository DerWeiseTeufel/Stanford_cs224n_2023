...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    y_w_dash = softmax(np.dot(outsideVectors,centerWordVec))
    y_w = np.zeros(outsideVectors.shape[0])
    y_w[outsideWordIdx] = 1
    loss = -1*np.dot(y_w, np.log(y_w_dash))
    gradCenterVec = np.dot(y_w_dash-y_w, outsideVectors)
    #print(np.atleast_2d(y_w_dash-y_w).T.shape )
    #print(np.atleast_2d(centerWordVec).shape)
    
    gradOutsideVecs = np.outer(y_w_dash - y_w, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    """
    loss = -1 * np.sum(np.log(sigmoid(np.dot(-1*outsideVectors[negSampleWordIndices],centerWordVec)))) - np.log(sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)))
    
    gradCenterVec = np.dot(outsideVectors[negSampleWordIndices].T,sigmoid(np.dot(outsideVectors[negSampleWordIndices], centerWordVec))) - sigmoid(np.dot(-1*outsideVectors[outsideWordIdx],centerWordVec) )*outsideVectors[outsideWordIdx]
    
    inds, counts = np.unique(indices, return_counts=True)
    outsideVectors[outsideWordIdx] = -outsideVectors[outsideWordIdx]
    zwischen = np.zeros(outsideVectors.shape[0])
    
    zwischen[inds] =sigmoid(np.dot(outsideVectors[inds], centerWordVec))
    #print(zwischen)
    zwischen[inds] *= counts
    #print(outsideWordIdx)
    #print(zwischen)
    #zwischen = np.append(zwischen, np.zeros(len(indices) - len(zwischen)))
    #print(counts[np.where(inds==outsideWordIdx)])
    
    outsideVectors[outsideWordIdx] = -outsideVectors[outsideWordIdx]
    zwischen[np.where(inds==outsideWordIdx)] *= -1
    gradOutsideVecs = np.outer(zwischen, centerWordVec)
    """
    #print(gradOutsideVecs)
    #print(outsideWordIdx,indices)
    #print(gradOutsideVecs.shape, outsideVectors.shape)
    #print(f'length of indices {len(indices)},zwischen {zwischen}, outsideVectors.shape {outsideVectors.shape}')
    #print(zwischen)
...
def negSamplingLossAndGradient_2(
    centerWordVec,
    outsideWordIdx,
    outsideVectors,
    dataset,
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    #print(windowSize - len(outsideWords)//2)
    c = word2Ind[currentCenterWord]
    for w_t in outsideWords:
        l1, g1, gout1 = word2vecLossAndGradient(centerWordVectors[c], word2Ind[w_t], outsideVectors, dataset)
        loss+=l1
        gradCenterVecs[c]+=g1
        gradOutsideVectors+=gout1
    ### END YOUR CODE
    #print(gradOutsideVectors)
